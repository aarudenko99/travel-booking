<form action="{{url( app_get_locale(false,false,'/').config('cruise.cruise_route_prefix') )}}"
        ...
        <div class="form-group form-date-field form-date-search clearfix  has-icon">
            ...
            <div class="date-wrapper clearfix">
                <div class="check-in-wrapper d-flex align-items-center">
                    <div class="render check-in-render">{{Request::query('start',display_date(strtotime("today")))}}
                    </div>
                    <span> - </span>
                    <div class="render check-out-render">{{Request::query('end',display_date(strtotime("+1 day")))}}
                    </div>
                </div>
            </div>
    ...
    <div class="filter-item filter-simple dropdown">
        ...
        <div class="filter-dropdown dropdown-menu dropdown-menu-right">
            <div class="bravo-filter-price">
                <?php
                $price_min = $pri_from = $cruise_min_max_price[0];
                $price_max = $pri_to = $cruise_min_max_price[1];
                ...
                <input type="hidden" class="filter-price irs-hidden-input" name="price_range"
                    data-symbol=" {{$currency['symbol'] ?? ''}}" data-min="{{$price_min}}" data-max="{{$price_max}}"
                    data-from="{{$pri_from}}" data-to="{{$pri_to}}" readonly="" value="{{$price_range}}">
