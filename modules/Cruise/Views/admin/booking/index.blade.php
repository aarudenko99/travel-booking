...
<div class="container-fluid">
    <div class="d-flex justify-content-between mb20">
        <h1 class="title-bar">{{__("Cruise Booking Calendar")}}</h1>
    </div>
    @include('admin.message')
    <div class="panel">
        {{--            <div class="panel-title"><strong>{{__("Cruise Filters")}}</strong></div>--}}
            ...
            <div class="col-left">
                <form method="get" action="" class="filter-form filter-form-left d-flex flex-column flex-sm-row"
                    ...
                    <select name="cat_id" class="form-control">
                        ...
                        <?php
                                foreach ($cruise_categories as $category) {
                                    $selected = '';
...
<div class="booking-calendar vec-wrap panel" id="booking-calendar">
    <div class="panel-body">
        <div class="vec-header-toolbar d-flex justify-content-between align-items-center">
            <div class=""><i><span
                        class="count-string">{{ __("Showing :from - :to of :total Cruise(s)",["from"=>$rows->firstItem(),"to"=>$rows->lastItem(),"total"=>$rows->total()]) }}</span></i>
            </div>
        ...
        <table class="vec-view-container" width="100%" cellpadding="0" cellspacing="0">
            <thead class="vec-head">
                <tr>
                    <th width="300px" class="vec-event-header">{{__('Cruises')}}</th>
                        ...
                        <div class="vec-scroll-flip">
                            <div class="vec-scroll-box">
                                <table class="" width="100%" cellpadding="0" cellspacing="0">
                                    <tr>
                                        @for($i = 1 ; $i <= date('t',$current_month) ; $i++) @php
                                            $day=strtotime(date('Y-m-',$current_month).$i) @endphp <td
                                            class="vec-time-text">
                                    ...
                                    <tr class="vec-event-list-tr">
                                        <td>
                                            <div class="vec-event-containers" data-id="{{$row->id}}">
                                                @php $bookings =
                                                $row->getBookingsInRange(date('Y-m-01',$current_month),date('Y-m-t',$current_month))
                                                @endphp
                                                @if(!empty($bookings))
                                                ...
                                                <div title="#{{$booking->id}} - {{$booking->email}}"
                                                    ...
                                                    data-toggle="modal" data-target="#modal-booking-{{$booking->id}}">
                                ...
                                <div class="vec-events-bg">
                                    <table class="" cellpadding="0" cellspacing="0" width="100%">
                                        <tr class="vec-event-time-row">
                                            @for($i = 1 ; $i <= date('t',$current_month) ; $i++) <td
                                                class="vec-event-time-td">
                                                <div>&nbsp;</div>
                    </td>
                    @endfor
                </tr>
