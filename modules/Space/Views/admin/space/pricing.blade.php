    ...
    <div class="panel-body">
        @if(is_default_lang())
            <div class="row">
                ...
                <div class="col-lg-6">
                    <div class="form-group">
                        <a href="{{ url("/user/space/availability") }}" class="control-label float-right" style="margin-top: 50px;" target="_blank">{{__("Add Price")}}</a>
                    </div>
                </div>
            </div>
        @endif
            ...
            <div class="g-items">
                @if(!empty($row->extra_price))
                    @foreach($row->extra_price as $key=>$extra_price)
                        @if(!is_default_lang())
                            <?php
                            $extra_price['name'] = $extra_price['name_'.request()->input('lang')] ?? $extra_price['name'];
                            ?>
                        @endif
