    ...
    public function loadDates(Request $request){
        ...
        for($i = strtotime($request->query('start')); $i <= strtotime($request->query('end')); $i+= DAY_IN_SECONDS)
        {
            ...
            if($space->default_state){
                ...
                $date['backgroundColor'] = '#75818c';
                $date['borderColor'] = '#75818c';
                $date['classNames'] = ['blocked-event', 'text-center'];
                $date['textColor'] = '#75818c';
        ...
        if(!empty($rows))
        {
            foreach ($rows as $row)
            {
                ...
                if(!$row->active)
                {
                    ...
                    $row->classNames = ['blocked-event', 'text-center'];
                    $row->textColor = '#fe2727';
                }else{
                    $row->classNames = ['active-event', 'text-center'];
                    if($row->is_instant){
