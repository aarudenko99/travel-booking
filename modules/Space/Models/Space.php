...
class Space extends Bookable
{
    ...
    public function addToCart(Request $request)
    {
        ...
        $total = $this->getPriceInRanges($request->input('start_date'),$request->input('end_date'),$request->input('service_id'));
    ...
    public function getPriceInRanges($start_date,$end_date,$target_id){
        $totalPrice = 0;
        $price = ($this->sale_price and $this->sale_price > 0 and  $this->sale_price < $this->price) ? $this->sale_price : $this->price;
        $datesRaw = $this->spaceDateClass::getDatesInRanges($start_date,$end_date,$target_id);
        $dates = [];
        if(!empty($datesRaw))
        {
